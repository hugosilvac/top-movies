...
class FavoriteViewController: UIViewController {
    ...
    private func bindViewModel() {
        ...
        let input = FavoriteViewModel.Input(trigger: Driver.merge(viewWillAppear, pull),
                                                 selection: collectionView.rx.itemSelected.asDriver())
        
        let output = favoriteViewModel.transform(input: input)
        ...
        let collectionViewFlow = sizeCollectionVerticalViewFlow(size: view.frame.width / CGFloat(3) - 8)
